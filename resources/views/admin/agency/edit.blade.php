...
@section('content')
    <div class="card w_120">
        <div class="card-header">
            <p><i class="fi fi-br-plus-small mr_15_icc"></i> Edit Agency</p>
        </div>
        {{-- {{ route('admin.asset-management.invoice.store') }} --}}
        <div class="card-body col-md-12">
            <form method="POST" action="{{ route('admin.aggency.update', [$edit_agency->id]) }}" enctype="multipart/form-data">
                @csrf
                <input type="hidden" name="id" value="{{ $edit_agency->id }}">
                <div class="row">
                    {{-- Dairy --}}
                    <div class="form-group">
                        <label class="required" for="name">{{ trans('cruds.agency.fields.name') }}</label>
                        <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text"
                            name="name" id="name" value="{{ old('name', $edit_agency->name) }}">
                        @if ($errors->has('name'))
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="text"
                            name="email" id="email" value="{{ old('email', $edit_agency->email) }}">
                        @if ($errors->has('email'))
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control {{ $errors->has('address') ? 'is-invalid' : '' }}" name="address" id="address"
                            rows="3">{{ old('address', $edit_agency->address) }}</textarea>
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('contact_no') ? 'is-invalid' : '' }}" type="text"
                            name="contact_no" id="contact_no" value="{{ old('contact_no', $edit_agency->contact_no) }}">
